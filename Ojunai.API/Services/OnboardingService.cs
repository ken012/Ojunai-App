...
/// <summary>
/// Multi-step conversational signup flow for new users messaging Ojunai via WhatsApp for the first time.
///
/// Flow:
...
public class OnboardingService
{
    ...
    public async Task<string?> HandleIfOnboardingAsync(string from, string phone, string text)
    {
        ...
        if (state == null)
        {
            ...
            {
                await Send(from, "Ojunai is a WhatsApp-first business management tool. You can record sales, track inventory, manage expenses, and more — all through chat.\n\nVisit ojunai.com to learn more, or reply *1* to create your business account.");
                return "onboarding:help";
            }

            await Send(from, "👋 Welcome to *Ojunai*!\n\nYour number isn't linked to a business yet.\n\nReply:\n*1* — Create a new business\n*2* — I was invited as staff\n*3* — Help");
    ...
    private async Task<bool> CreateAccountAsync(string from, string phone, OnboardingState state)
    {
        ...
        if (phoneExists)
        {
            _db.OnboardingStates.Remove(state);
            await _db.SaveChangesAsync();
            await Send(from, "This phone number is already registered. Try logging into the dashboard at app.ojunai.com");
            return false;
        }
